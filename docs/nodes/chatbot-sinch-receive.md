...
You should get an HTTPS address like _https://123123.ngrok.io_ — this is the base URL that points back to your **Node-RED** instance.
...
1. In the dashboard, open **Settings → Access keys** (top-right user menu, or the project _Settings_ section).
2. Click **Create new key**. Give it a label (e.g. _"RedBot dev"_) and confirm.
3. Sinch will show you the **Key ID** and the **Key Secret** _only once_ — copy both immediately and store them somewhere safe. If you lose the secret you'll have to generate a new key.
...
Back in the Conversation API app (**Conversation API → Apps → _your app_**):

1. Open the **Webhooks** section and click **New webhook** (or _Add webhook_).
2. Set the **Target URL** to the ngrok address you grabbed earlier, ending with `/redbot/sinch`, e.g.

   ```plain text
   https://123123.ngrok.io/redbot/sinch
   ```

3. Under **Triggers**, enable at least `MESSAGE_INBOUND` so that messages coming from end users are delivered to RedBot. You can also enable `MESSAGE_DELIVERY`, `EVENT_INBOUND`, `EVENT_DELIVERY`, `CONVERSATION_START` and `CONVERSATION_STOP` if your flow needs them.
4. Save the webhook.
