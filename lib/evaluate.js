...
module.exports = function(msg, node = null) {
  ...
  const extractObjectKeys = function(value, subtokens) {
    ...
    if (_.isArray(subtokens) && !_.isEmpty(subtokens)) {
      _.each(subtokens, function(subtoken) {
  ...
  return function(message) {
    ...
    // extract tokens (include cirillic)
    let tokens = message.match(/\{\{([A-Za-z0-9\-\.аАбБвВгГдДеЕёЁжЖзЗиИйЙкКлЛмМнНоОпПрРсСтТуУфФхХцЦчЧшШщЩъЪыЫьЬэЭюЮяЯ]*?)\}\}/g);
    if (tokens != null && tokens.length != 0) {
      tokens = _.map(tokens, token => token.replace('{{', '').replace('}}', ''));
    }
    // replace them
