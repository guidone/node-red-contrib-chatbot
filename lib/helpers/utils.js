...
function checkValidators(types, name, value) {
  return _.some(types, function(type) {
    ...
    switch(type) {
      ...
      case 'arrayOfString':
        validator = function(value) {
          return _.isArray(value) && _.every(value,function(obj) {
      ...
      case 'buttons':
        validator = function(value) {
          return _.isArray(value) && !_.isEmpty(value) && _.every(value,function(button) {
      ...
      case 'arrayOfEntities':
        validator = function(value) {
          return _.isArray(value) && !_.isEmpty(value) && _.every(value,entity => {
            return entity != null && !_.isEmpty(entity.name) &&
              (entity.aliases == null || (_.isArray(entity.aliases) && _.every(entity.aliases, alias => _.isString(alias))));
          });
        };
        break;
      case 'arrayOfFacebookTemplateElements':
        validator = function(value) {
          return _.isArray(value) && _.every(value,(obj) => {
      ...
      case 'whatsappTemplateParams':
        validator = function(value) {
          return _.isArray(value) && !_.isEmpty(value) && _.every(value,(obj) => {
...
const utils = {
  ...
  /**
   * @method request
   * HTTP helper built on global `fetch`. Accepts a subset of the legacy `request`
   * library options used across the codebase: `url`/`uri`, `method`, `headers`,
   * `qs` (query params), `body`, `json` (`true` to parse the response as JSON,
   * an object to also send it as a JSON body) and `formData` (multipart, with
   * entries shaped either as plain values or `{ value, options: { filename, contentType } }`).
   * Resolves with a Buffer by default, or the parsed JSON body when `json` is set.
   */
  async request(opts) {
    let finalUrl = opts.url || opts.uri;
    if (opts.qs) {
      const qs = new URLSearchParams(opts.qs).toString();
    ...
    if (opts.json != null && typeof opts.json === 'object') {
      fetchOpts.headers['Content-Type'] = 'application/json';
      fetchOpts.body = JSON.stringify(opts.json);
    } else if (opts.formData) {
      const fd = new FormData();
      for (const [key, value] of Object.entries(opts.formData)) {
        if (value && typeof value === 'object' && value.value !== undefined && value.options) {
          const blob = new Blob([value.value], { type: value.options.contentType });
          fd.append(key, blob, value.options.filename);
        } else {
          fd.append(key, value);
        }
      }
    ...
    const wantsJson = opts.json === true || (opts.json != null && typeof opts.json === 'object');
    if (wantsJson) {
  ...
  /**
   * isSkipValidation
   * Tells to skip validation in message factory
   * @param {Object} msg
   * @returns {Boolean}
   */
  isSkipValidation(msg) {
    return msg.payload?.skipValidation;
  },
  ...
  matchTransport(node, msg) {
    ...
    if (!_.includes(transports, utils.getTransport(msg))) {
