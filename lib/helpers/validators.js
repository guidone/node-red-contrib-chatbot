...
var validators = {
  ...
  platform: {
    ...
    deepchat: function(config) {
      config = config || {};
      var result = {};
      if (!_.isString(config.botId) || _.isEmpty(config.botId)) {
        result.botId = 'Missing or invalid botId';
      } else if (config.botId.match(/^[A-Za-z0-9_-]+$/) == null) {
        result.botId = 'Invalid botId, only letters, digits, "-" and "_" are allowed';
      }
      if (config.timeout != null && !_.isNumber(config.timeout)) {
        result.timeout = 'Must be a number';
      }
      if (config.collectWindow != null && !_.isNumber(config.collectWindow)) {
        result.collectWindow = 'Must be a number';
      }
      if (config.publicUrl != null && !_.isEmpty(config.publicUrl) && !validators.url(config.publicUrl)) {
        result.publicUrl = 'Invalid Url';
      }
      _.extend(result, validators.platform.contextProvider(config));
      return _.keys(result).length === 0 ? null : result;
    },
    ...
    slack: function(config) {
      ...
      if (!_.isString(config.signingSecret) || _.isEmpty(config.signingSecret)) {
        result.signingSecret = 'Missing or invalid signing secret';
      }
      if (!_.isString(config.serverPort) || _.isEmpty(config.serverPort)) {
        result.serverPort = 'Missing or invalid server port';
      }
      _.extend(result, validators.platform.contextProvider(config));
      return _.keys(result).length === 0 ? null : result;
    },
    ...
    sinch: function(config) {
      config = config || {};
      var result = {};
      if (!_.isString(config.projectId) || _.isEmpty(config.projectId)) {
        result.projectId = 'Missing or invalid projectId';
      }
      if (!_.isString(config.appId) || _.isEmpty(config.appId)) {
        result.appId = 'Missing or invalid appId';
      }
      if (!_.isString(config.keyId) || _.isEmpty(config.keyId)) {
        result.keyId = 'Missing or invalid keyId';
      }
      if (!_.isString(config.keySecret) || _.isEmpty(config.keySecret)) {
        result.keySecret = 'Missing or invalid keySecret';
      }
      _.extend(result, validators.platform.contextProvider(config));
      return _.keys(result).length === 0 ? null : result;
    },
    telegram: function(config) {
      ...
      if (config.connectMode != null && !_.includes(['polling', 'webHook'], config.connectMode)) {
    ...
    contextProvider: function(config) {
      ...
      if (config.contextProvider != null && !_.includes(allowedContextProviders, config.contextProvider)) {
  ...
  shippingOptions: function(elements) {
    return _.isArray(elements)
      && !_.isEmpty(elements)
      && _.every(elements, function(element) {
  ...
  buttons: function(buttons) {
    return _.isArray(buttons)
      && !_.isEmpty(buttons)
      && _.every(buttons, function(button) {
  ...
  genericTemplateElements: function(elements) {
    return _.isArray(elements)
      && !_.isEmpty(elements)
      && _.every(elements, element => validators.genericTemplateElement(element));
  },
  ...
  productTemplateElements: function(elements) {
    return _.isArray(elements)
      && !_.isEmpty(elements)
      && _.every(elements, element => validators.productTemplateElement(element));
  },

  productTemplateElement: function(element) {
    return _.isObject(element) && element.id != null;
  },

  buttonTemplateElements: function(elements) {
    return _.isArray(elements)
      && !_.isEmpty(elements)
      && _.every(elements, element => validators.buttonTemplateElement(element));
  },

  buttonTemplateElement: function(element) {
    return _.isObject(element)
      && !_.isEmpty(element.text)
      && _.isString(element.text)
      && _.isArray(element.buttons)
      && (element.buttons.length === 0 || validators.buttons(element.buttons));
  },
  ...
  nlpTokens: function(tokens) {
    return !_.isEmpty(tokens) && _.every(tokens.split(','), function(token) {
  ...
  messages: function(value) {
    return !_.isEmpty(value) && _.isArray(value) && _.every(value, function(message) {
  ...
  /**
   * arrayOfMessage
   * Test if the value is a valid RedBog message (the format the nodes exchange between them), useful to tell if the computed
   * message is to be chained with the inbound payload or it should be just replaced
   * @param {Any} value
   */
  arrayOfMessage: function(value) {
    return _.isArray(value) && _.every(value, item => _.isObject(item) && item.inbound === false);
  },
