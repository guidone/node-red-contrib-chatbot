...
var validators = {
  ...
  platform: {
    discord: function(config) {
      config = config || {};
      var result = {};
      if (config.usernames != null && !_.isString(config.usernames)) {
        result.usernames = 'Must be a comma separated list of chatIds';
      }
    ...
    telegram: function(config) {
      ...
      if (config.usernames != null && !_.isString(config.usernames)) {
        result.usernames = 'Must be a comma separated list of username or chatIds';
      }
      if (config.logfile != null && !_.isString(config.logfile)) {
        result.logfile = 'Must be a valid filename';
      }
  ...
  genericTemplateElements: function(elements) {
    return _.isArray(elements)
      && !_.isEmpty(elements)
      && _(elements).all(element => validators.genericTemplateElement(element));
  },
  ...
  productTemplateElements: function(elements) {
    return _.isArray(elements)
      && !_.isEmpty(elements)
      && _(elements).all(element => validators.productTemplateElement(element));
  },
  ...
  buttonTemplateElements: function(elements) {
    return _.isArray(elements)
      && !_.isEmpty(elements)
      && _(elements).all(element => validators.buttonTemplateElement(element));
  },
  ...
  arrayOfMessage: function(value) {
    return _.isArray(value) && _(value).all(item => _.isObject(item) && item.inbound === false);
  },
