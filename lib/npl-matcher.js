...
function groupSiblings(terms, type, excludedTypes) {
  ...
  for(k = 0; k < terms.length; k++) {
    ...
    /*if (type == 'Date') {
      console.log('- ??', term._text);
      console.log('- type', type, term.tags[type] != null);
      if (nextTerm != null && nextTerm.tags[type] != null) {
        console.log('-', _(nextTerm.tags).keys(), excludedTypes);
      }
    }*/
    if (term.tags[type]) {
      temp.push(term);
      ...
      var isExclude = nextTerm != null ? !_.isEmpty(_.intersection(_(nextTerm.tags).keys(), excludedTypes)) : false;
      console.log('isExclude', isExclude);
      if (isLast || !nextTerm.tags[type] || isExclude) {
        if (temp.length > 0) {
...
module.exports = {
  ...
  parseSentence: function(str, lexicon, debug) {
    ...
    // join Date elements but only if they are not "Unit", for example "in 8 minutes" could be grouped all in a single
    // Date element, but since "in" is a preposition and "minutes" is unit
