...
_.extend(MatchRule.prototype, {
  ...
  _matchingRules: [
    ...
    function(term) {
      ...
      if (term.tags[capitalizedType] || this.type === 'word') {
    ...
    function(term) {
      ...
      if (term.tags.Cardinal) {
        //if (this.type == 'number') {
          /*if (term.tags.Cardinal) {
            this.value = parseInt(term.normal, 10);
          }*/
...
module.exports = {
   ...
   * @return {Array}
   */
  parseSentence: function(str, lexicon) {
    ...
    terms = groupSiblings(terms, 'Date');

    //console.log('FINALE', terms);
...
function groupSiblings(terms, type) {
  ...
  terms.forEach(function(term) {
    // add to the temp registry if it's the right type
    if (term.tags[type]) {
      temp.push(term);
    }

    if ((!term.tags[type] || term.uid === lastTerm.uid) && temp.length !== 0) {
      ...
      switch(type) {
        case 'Date':
          ...
          //temp[0].infinitive = parsed[0].conjugations.Infinitive;
          temp[0]._text = joined;
