  ...
  _matchingRules: [
    function(term) {
      ...
      if (matchType) {
        if (!_.isEmpty(this.text)) {
          // todo check lower case and lehvestein
    ...
    function(term) {
      ...
      if (this.type === 'verb' && term.pos.Verb) {
        if (!_.isEmpty(this.text)) {
          //console.log('passa verbo?', this.text === term.text || this.text == term.root());
          return this.text === term.text || this.text == term.root();
        } else {
          // catches all
          this.value = term.text;
          return true;
...
module.exports = {
  ...
  parseRules: function(rules) {
    // todo convert string rules to object rules
  },
