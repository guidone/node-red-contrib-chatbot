...
var Alexa = new ChatExpress({
  ...
  routes: {
    '/redbot/alexa': function(req, res) {
      ...
      if (req.body.request != null && req.body.request.intent != null) {
        console.log('+++++++++++++++++++++++++++++++');
        console.log(prettyjson.render(req.body.request.intent));
        console.log('+++++++++++++++++++++++++++++++');
      }
...
Alexa.in(function(message) {
  ...
  return new Promise(function(resolve) {
    ...
    if (payload.request != null && payload.request.type === 'IntentRequest') {
      ...
      // set the dialog state and confirmation
      if (!_.isEmpty(payload.request.intent.confirmationStatus)) {
      ...
      _(payload.request.intent.slots)
        .each(function(slot) {
          ...
          message.payload.slotConfirmationStatus[slot.name] = _.isString(slot.confirmationStatus) ?
            slot.confirmationStatus.toLowerCase() : 'none';
        });
      ...
      console.log('------INTENT');
      console.log(prettyjson.render(_.omit(message.payload, 'ts')));
      console.log('------');
    ...
    } else if (payload.request != null && payload.request.type === 'SessionEndedRequest') {
      // bot is started
      message.payload.type = 'event';
...
Alexa.out(function(message) {
  ...
  return new Promise(function(resolve) {
    ...
    console.log(prettyjson.render(alexaMessage));
    console.log('----------------------');*/
