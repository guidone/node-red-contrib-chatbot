...
const { renderButtons, renderLocation, renderWaiting, renderFile } = require('./renderer');
...
/**
 * The *Waiting* node. There's no status line to set on the platform (Deep Chat is the platform): the
 * wait is a temporary message the widget drops as soon as the answer arrives, see `renderWaiting()`.
 *
 * It's only sent in WebSocket mode. In http mode the widget is already showing the loading indicator of
 * the request it's waiting on, and it's the only moment it can be reached: a wait pushed into the
 * response bridge would either close the collecting window before the real answer is ready, or surface
 * on the next round trip, long after the flow stopped waiting
 */
DeepChat.out('action', async function(message) {
  if (this.sockets == null) {
    return message;
  }
  return push(this, message, renderWaiting(message.payload));
});
...
DeepChat.registerMessageType('action', 'Action', 'Send an action message (like typing, ...)');
