...
<body>
  <header>
    ...
    <div class="toolbar">
      ...
      <button class="reset" id="open-codepen">Open in CodePen</button>
    </div>
  </header>
  <main>
  ...
  <details class="embed">
    ...
    <p class="hint">
      <strong>Open in CodePen</strong> sends the same markup to codepen.io. A pen runs on
      <code>https://cdpn.io</code>, so it reaches this bot only if the url above is reachable from the
  ...
  <script type="module">
    ...
    document.getElementById('open-codepen').addEventListener('click', function() {
      const pen = {
        title: botname + ' \\u00b7 RedBot Deep Chat',
      ...
      // a prefill is a POST, so it can't be a plain link
