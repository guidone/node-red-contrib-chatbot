...
const { when, params, request } = require('../../helpers/utils');
...
const Facebook = new ChatExpress({
  ...
  tsKey: function(payload) {
    return dayjs.unix(payload.timestamp / 1000).toISOString();
  },
  type: function() {
    // todo remove this
  },
...
Facebook.out('action', async function(message) {
  ...
  await request({
    ...
    url: `${FACEBOOK_API_URL}/me/messages?access_token=` + options.token
  });
  return message;
});
...
Facebook.mixin({
  ...
  sendVerificationChallenge: function(req, res) {
    ...
    log(
      `Verifying Facebook Messenger token ${query.get('hub.verify_token')}, should be `
        + (options.verifyToken ? `"${options.verifyToken}"` : 'anything')
    );
    log('Token verified.');
    return res.end(query.get('hub.challenge'));
    /*if (query.get('hub.verify_token') === options.verifyToken) {
  ...
  */
  ...
  sendMessage: async function(recipient, payload, opts = {}) {
    const options = this.getOptions();
  ...
  removePersistentMenu: async function() {
    var options = this.getOptions();
    const body = await request({
      method: 'DELETE',
  ...
  setPersistentMenu: async function(items, composerInputDisabled) {
    const options = this.getOptions();
    const body = await request({
      method: 'POST',
      uri: `${FACEBOOK_API_URL}/me/messenger_profile`,
