    ...
    task
      .then(function(obj) {
        ...
        if (_.isArray(authorizedUsernames) && !_.isEmpty(authorizedUsernames)) {
          if (_.contains(authorizedUsernames, String(userId))) {
            obj.authorized = true;
          }
        }
        return obj;
...
Facebook.out('inline-buttons', function(message) {
  var chatServer = this;
  return new Promise(function(resolve, reject) {
    chatServer.sendMessage(message.payload.chatId, payloadTx.inlineButtons(message.payload))
      .then(function() {
        resolve(message);
      },function(err) {
        reject(err);
...
Facebook.out('list-template', function(message) {
  ...
  return new Promise(function (resolve, reject) {
    chatServer
      .sendMessage(message.payload.chatId,payloadTx.listTemplate(message.payload))
      .then(
        function() {
          resolve(message);
        },
...
Facebook.out('template', function(message) {
  ...
  return new Promise((resolve, reject) => {
    ...
    chatServer
      .sendMessage(message.payload.chatId, payload)
      .then(
        () => resolve(message),
        error => reject(error)
      );
