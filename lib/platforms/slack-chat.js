...
Slack.in('photo', function(message) {
  var chatServer = this;
  return new Promise(function(resolve, reject) {
    chatServer.downloadUrl(message.originalMessage.files[0].url_private_download)
      .then(
        function(body) {
        ...
        function() {
          reject('Error loading: ' + message.payload.files[0].url_private_download);
        });
  });
});
...
// todo only show badges for platforms actually visible (no telegram for example)
