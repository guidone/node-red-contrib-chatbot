...
const Slack = new ChatExpress({
  ...
  onStop: async function() {
    ...
    if (chatServer.slackEvents != null) {
      await chatServer.slackEvents.stop();
    }
    return true;
  },
  onStart: async function() {
    ...
    // TODO move to config
    // TODO remove oauth token non serve piu
    const slackEvents = createEventAdapter('636d2296f541ce437dfe208e3c410507');
    ...
    console.log('Starting Slack Event server')

    await slackEvents.start(3001);

    // Log a message when the server is ready
    console.log(`Listening for events on 3001`);




    chatServer.slackEvents = slackEvents;
