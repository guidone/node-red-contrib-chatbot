  ...
  return async function({ next, body: payload, client, ack }) {
    ...
    if (!_.isEmpty(payload.command)) {
      ...
      chatServer.receive({
        ...
        ts: dayjs.unix(payload.event.ts).toISOString(),
      });
      ...
      chatServer.receive({
        ...
        ts: dayjs.unix(payload.action_ts).toISOString(),
...
const Slack = new ChatExpress({
  ...
  tsKey: function(payload) {
    return dayjs.unix(payload.ts).toISOString();
  },
...
Slack.out('dialog', async function(message) {
  ...
  const elements = _.map(message.payload.elements, function(item) {
...
Slack.out('generic-template', async function(message) {
  ...
  const client = options.client;
  const attachments = _.map(message.payload.elements, function(item) {
    var attachment = {
      title: item.title,
...
Slack.mixin({
  ...
  downloadUrl: function(url) {
    ...
    if (_.isEmpty(node_options.token)) {
      ...
      return Promise.reject('No OAuth Token configured. Check');
    }
    return request({
      url: url,
      headers: { 'Authorization': 'Bearer ' + node_options.token }
    }).catch(function() {
      return Promise.reject('Unable to download file ' + url);
    });
  },
