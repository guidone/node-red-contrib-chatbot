...
features:
  app_home:
    ...
    messages_tab_read_only_enabled: false
  bot_user:
    display_name: Test Slack No WS
    always_online: true
  slash_commands:
    - command: /test
      url: https://a514e8928e0d.ngrok.io/slack/events
      description: Test command
      usage_hint: Test command hint
      should_escape: false
oauth_config:
  scopes:
    bot:
...
settings:
  event_subscriptions:
    request_url: https://a514e8928e0d.ngrok.io/slack/events
    bot_events:
      - message.channels
      - message.im
  interactivity:
...
/*const commandHandler = function(req, res) {
  this.receive({
    type: 'message',
...
const boltMiddleware = (chatServer) => {
  return async function({ next, body: payload, client, ack }) {
    ...
    if (!_.isEmpty(payload.command)) {
      ...
      // TODO intercept escaped commands
      chatServer.receive({
        ...payload.event,
...
const Slack = new ChatExpress({
  ...
  onStart: async function() {
    ...
    // TODO escaped commands...
      ...
      app.use(bodyParser());
      app.use('/interactions', webhookHandler.bind(chatServer));
      app.use('/commands', commandHandler.bind(chatServer));
      app.use('/test', (req, res) => res.send('ok'));

      slackEvents.on('message', message => {
        if (message.bot_profile != null) {
          // do nothing, this is a message sent by WebClient, there's nothing other than thatkey to tell the difference
          // between a message received from the bot or sent to the bot, sorry Slack this is really a shitty developer
          // experience, 3 different libs to do the same thing, breaking changes in API and permissions, 3 different tokens,
          // what's next? ...
          return;
        }
        // slack hai its own command, remove escape //my_command -> /my_command
        if (message.type === 'message' && message.text != null) {
          message.text = escapeSlackCommands(message.text);
        }
        chatServer.receive(message);
      });
      // create express server
      chatServer.server = createServer(app);
      chatServer.server.listen(serverPort, () => {
...
Slack.out('message', async function(message) {
  ...
  await chatContext.set('messageId', result.ts);
  return message;
});
...
Slack.out('location', function(message) {
  var options = this.getOptions();
  var slackExtensions = this.getSlackExtensions(message);
  return new Promise(function(resolve, reject) {
    var client = options.client;
    // build map link
