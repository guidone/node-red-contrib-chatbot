...
// node-telegram-bot-api is ESM-only since v1.0.0, so it cannot be loaded with a
// synchronous require() on Node < 22. Load it lazily via dynamic import() instead
// (the class is only instantiated inside the async onStart handler).
let TelegramBot;
const getTelegramBot = async () => {
  if (TelegramBot == null) {
...
const Telegram = new ChatExpress({
  ...
  tsKey: function(payload) {
    return dayjs.unix(payload.date).toISOString();
  },
  type: function() {
    // todo remove this
  },
  ...
  routes: {
    ...
    '/redbot/telegram': function(req, res) {
      ...
      if (json.message != null) {
        ...
        json.edited_message._edited = true;
        chatServer.receive(json.edited_message);
      } else if (json.channel_post != null) {
        chatServer.receive(json.channel_post);
      } else if (json.edited_channel_post != null) {
        json.edited_channel_post._edited = true;
        chatServer.receive(json.edited_channel_post);
      } else if (json.callback_query != null) {
        handleCallbackQuery.call(chatServer, json.callback_query);
      } else if (json.inline_query != null) {
        handleInlineQuery.call(chatServer, json.inline_query);
      } else if (json.pre_checkout_query != null) {
        handlePreCheckoutQuery.call(chatServer, json.pre_checkout_query);
      } else if (json.shipping_query != null) {
        handleShippingQuery.call(chatServer, json.shipping_query);
      }
      res.send({ status: 'ok' });
    }
  },
  ...
  events: {
    inline_query: function(botMsg) { handleInlineQuery.call(this, botMsg); },
    callback_query: function(botMsg) { handleCallbackQuery.call(this, botMsg); },
    pre_checkout_query: function(botMsg) { handlePreCheckoutQuery.call(this, botMsg); },
    shipping_query: function(botMsg) { handleShippingQuery.call(this, botMsg); },
    channel_post: function(botMsg) { this.receive(botMsg); },
    edited_channel_post: function(botMsg) {
      botMsg._edited = true;
      this.receive(botMsg);
    },
    edited_message: function(botMsg) {
      botMsg._edited = true;
      this.receive(botMsg);
    }
  }
...
// Shared event handlers — used by both webhook routes and polling events
function handleInlineQuery(botMsg) {
  botMsg.inlineQueryId = botMsg.id;
  delete botMsg.id;
  this.receive(botMsg);
}

function handleCallbackQuery(botMsg) {
  this.callbackQuery(botMsg);
}

function handlePreCheckoutQuery(botMsg) {
  const chatServer = this;
...
function handleShippingQuery(botMsg) {
  this.receive({
    from: botMsg.from,
...
// mark edited messages so downstream nodes can distinguish them
Telegram.in(function(message) {
  if (message.originalMessage._edited === true) {
    message.payload.edited = true;
  }
  return Promise.resolve(message);
});

// propagate generic Telegram metadata fields (reply, forwarding, topics, business)
Telegram.in(function(message) {
  const botMsg = message.originalMessage;
  if (botMsg.reply_to_message != null) {
    message.payload.replyToMessage = botMsg.reply_to_message;
  }
  if (botMsg.forward_origin != null) {
    message.payload.forwardOrigin = botMsg.forward_origin;
  }
  if (botMsg.message_thread_id != null) {
    message.payload.messageThreadId = botMsg.message_thread_id;
  }
  if (botMsg.business_connection_id != null) {
    message.payload.businessConnectionId = botMsg.business_connection_id;
  }
  return Promise.resolve(message);
});
...
// detect user leaving group
Telegram.in(function(message) {
  var botMsg = message.originalMessage;
  return new Promise(function(resolve) {
    if (botMsg.left_chat_member != null) {
      message.payload.leftUser = botMsg.left_chat_member;
      message.payload.type = 'event';
      message.payload.eventType = 'user-left';
      resolve(message);
    } else {
      resolve(message);
    }
...
Telegram.in(function(message) {
  ...
  return new Promise(function(resolve) {
    if (botMsg.dice != null) {
      message.payload.content = botMsg.dice;
      message.payload.type = 'dice';
      resolve(message);
    } else {
      resolve(message);
    }
...
Telegram.in(function(message) {
  ...
  return new Promise(function(resolve) {
    if (botMsg.poll != null) {
      message.payload.content = botMsg.poll;
      message.payload.type = 'poll';
      resolve(message);
    } else {
      resolve(message);
    }
...
// detect venue (location with title/address)
Telegram.in(function(message) {
  var botMsg = message.originalMessage;
  return new Promise(function(resolve) {
    if (botMsg.venue != null) {
      message.payload.content = botMsg.venue;
      message.payload.type = 'venue';
      resolve(message);
    } else {
      resolve(message);
    }
...
Telegram.in(async function(message) {
  ...
  // note: order matters — animation arrives with a `document` sibling, so test it first
  if (botMsg.animation != null) {
    type = 'animation';
    fileId = botMsg.animation.file_id;
    duration = botMsg.animation.duration;
  } else if (botMsg.photo != null) {
    type = 'photo';
    fileId = _.last(botMsg.photo).file_id;
  ...
  } else if (botMsg.audio != null) {
    type = 'audio-file';
    fileId = botMsg.audio.file_id;
    duration = botMsg.audio.duration;
  ...
  if (type != null) {
    ...
    try {
      ...
      if (botMsg.caption_entities != null) {
        message.payload.captionEntities = botMsg.caption_entities;
      }
...
Telegram.out('photo', async function(message) {
  ...
  const result = await connector.sendPhoto(message.payload.chatId, message.payload.content, {
      ...
      show_caption_above_media: param('showCaptionAboveMedia', false),
...
Telegram.out('telegram-buttons', async function(message) {
  ...
  const keyboard = [[]];
  _.each(message.payload.buttons, button => {
    let json = null;
    switch(button.type) {
      case 'keyboardButton':
        json = button.label;
...
Telegram.out('message', async function(message) {
  ...
  if (modifyMessageId != null) {
    ...
    for(let idx = 0; idx < chunks.length; idx++) {
      // simulate spoiler with entities — Telegram ignores parse_mode when entities are set,
      // so we drop it here to avoid the surprise of silently-stripped formatting
      ...
      result = await connector.sendMessage(message.payload.chatId, chunks[idx], {
        parse_mode: hasSpoiler ? undefined : param('parseMode', null),
        disable_web_page_preview: param('disableWebPagePreview', false),
        disable_notification: param('silent', false),
        reply_parameters: param('replyToMessage', false) ? { message_id: getMessageId(message) } : undefined,
        protect_content: param('protectContent', false),
...
Telegram.out('invoice', async function(message) {
  ...
  const invoiceOptions = {
    ...
    protect_content: param('protectContent', false),
    start_parameter: message.payload.startParameter,
    message_effect_id: param('messageEffectId', null),
  };
  const prices = _.map(message.payload.prices, price => {
    return {
      label: price.label,
...
Telegram.out('invoice-shipping', async function(message) {
  ...
  const shippingOptions = _.map(message.payload.shippingOptions, function(shippingOption) {
    return {
...
Telegram.mixin({
  downloadFile: async function (url, token) {
  ...
  callbackQuery: function(botMsg) {
    ...
    if (connector.lastInlineButtons != null && connector.lastInlineButtons[chatId] != null) {
      ...
      var button = _.find(connector.lastInlineButtons[chatId], {value: botMsg.data});
      if (button != null) {
...
Telegram.registerParam(
  'allowPaidBroadcast',
  'boolean',
  { label: 'Allow paid broadcast', default: false, description: 'Allow up to 1000 messages per second in paid private channels' }
);
Telegram.registerParam(
  'hasSpoiler',
  'boolean',
...
Telegram.registerParam(
  ...
  'boolean',
  { label: 'Show caption above media', default: false, description: 'Show the caption above the photo instead of below' }
);
Telegram.registerParam(
  'protectContent',
  'boolean',
...
Telegram.registerParam(
  ...
  'select',
  {
    ...
    default: null,
    description: 'Visual effect animation shown when the message is received (private chats only)',
    options: [
      { value: null, label: 'None' },
      { value: '5107584321108051014', label: '👍 Like' },
      { value: '5159385139981059251', label: '❤️ Heart' },
      { value: '5104841245755180586', label: '🔥 Fire' },
      { value: '5104858069142078462', label: '👎 Dislike' },
...
Telegram.registerMessageType('dice', 'Dice', 'Receive a dice/emoji animation');
Telegram.registerMessageType('poll', 'Poll', 'Receive a poll');
Telegram.registerMessageType('venue', 'Venue', 'Receive a venue (location with title/address)');
Telegram.registerMessageType('audio-file', 'Audio File', 'Receive a music/audio file (not a voice note)');

Telegram.registerEvent('new-user', 'New user');
Telegram.registerEvent('user-left', 'User left');
