...
const Telegram = new ChatExpress({
  ...
  chatIdKey: function(payload) {
    if (payload.chat != null && payload.chat.id != null) {
      return payload.chat.id;
    } else if (payload.user != null && payload.user.id != null) {
      return payload.user.id;
    } 
    return null;
  },
  userIdKey: function(payload) {
    if (payload.from != null && payload.from.id != null) {
  ...
  routes: {
    ...
    '/redbot/telegram': function(req, res) {
      ...
      const json = req.body;
      console.log('cosa arriva con web hook', json)
      ...
      } else if (json.poll_answer != null) {            
        this.receive(json.poll_answer);
...
// detect non anonymous poll answer
Telegram.in(function(message) {
  const botMsg = message.originalMessage;
  const connector = message.client();

  return new Promise((resolve, reject) => {
    if (!_.isEmpty(botMsg.poll_id)) {
      const prefefinedAnswers = connector.lastPolls[botMsg.poll_id];
      if (connector.lastPolls != null && prefefinedAnswers != null) {      
        message.payload.type = 'event';
        message.payload.eventType = 'poll';      
        // pick up answer
        let answers = [];
        botMsg.option_ids.forEach(idx => answers.push(prefefinedAnswers[idx]));
        message.payload.answers = answers;
        resolve(message);
      } else {
        reject(`Poll with id ${botMsg.poll_id} not found`);
      }    
    } else {
      resolve(message);
    }
...
/*
Anonymous polls in chatbots don't make sense, only groups
Telegram.in(function(message) {
  const botMsg = message.originalMessage;
  return new Promise(resolve => {
    if (!_.isEmpty(botMsg.poll)) {        
      message.payload.type = 'event';
      message.payload.eventType = 'poll';      
      // pick up answer
      let answers = [];
      botMsg.poll.options.forEach(vote => {
        if (vote.voter_count !== 0) {
          answers.push(vote.text);
        }
      });
      message.payload.answers = answers;
      resolve(message);
...
Telegram.in(function(message) {
  ...
  if (from != null) {
    ...
    if (!_.isEmpty(from.first_name)) {
      vars.firstName = from.first_name;
    }
    if (!_.isEmpty(from.last_name)) {
      vars.lastName = from.last_name;
    }
  }
...
Telegram.out('poll', function(message) {
  //const options = this.getOptions();
  const connector = this.getOptions().connector;
  const param = params(message);
  ...
  return connector.sendPoll(
    ...
    // store the last poll, this will be handled by the receiver
    if (connector.lastPolls == null) {
      connector.lastPolls = {};
    }
    connector.lastPolls[resp.poll.id] = options;
    return message
  });
});
...
Telegram.registerMessageType('poll', 'Poll', 'Send a multiple choice answer to the user');
...
Telegram.registerEvent('poll', 'Sent when a user answer to a poll');
