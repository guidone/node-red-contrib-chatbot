        ...
        .each(partial => {
          task = task.then(() => 
            connector.sendMessage(message.payload.chatId, partial, {
              ...
              reply_to_message_id: payload.options != null ? payload.options.reply_to_message_id : null
            })
          );
        });
    }
...
Telegram.out('sticker', function(message) {
  ...
  return connector.sendSticker(message.payload.chatId, message.payload.content)
    .then(() => message);
});
...
Telegram.out('animation', function(message) {
  ...
  return connector.sendAnimation(message.payload.chatId, message.payload.content)
    .then(() => message);
});
...
Telegram.registerParam(
  'replyToMessage',
  'boolean',
  { label: 'Reply to previous message', default: false }
);
Telegram.registerParam(
  'fromNumber',
  'string',
