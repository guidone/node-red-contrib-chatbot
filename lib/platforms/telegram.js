...
const Telegram = new ChatExpress({
  ...
  chatIdKey: function(payload) {
    console.log('CHATID PAYLOAD', payload);
    console.log('--', payload.chat != null ? payload.chat.id : null)
  ...
  events: {
    ...
    poll: function(botMsg) {
      console.log('anon poll', botMsg)
...
Telegram.in(function(message) {
  ...
  console.log('qua ci passa?', botMsg)
  return new Promise(function(resolve) {
    if (!_.isEmpty(botMsg.poll_id)) {
      //message.payload.newUsers = botMsg.new_chat_members;
      message.payload.type = 'poll-answer';
      message.payload.content = 'risposta';
      //message.payload.eventType = 'new-user';
      console.log('botMsg.poll_answer', botMsg)
      console.log('--', connector.lastPolls[botMsg.poll_id])
      resolve(message);
    } else {
      resolve(message);
    }
...
Telegram.in(function(message) {
  ...
  const userId = message.originalMessage.from != null ?
    String(message.originalMessage.from.id) : null;
  const username = message.originalMessage.from != null ? 
    String(message.originalMessage.from.username) : null;
  const vars = {};


  // TODO check here if the from is empty
  return new Promise(function(resolve, reject) {
...
Telegram.out('poll', function(message) {
  ...
  const { chatId, title, options, isAnonymous, isQuiz, answer } = message.payload;
  console.log
