...
const Whatsapp = new ChatExpress({
  ...
  tsKey: function(payload) {
    return new Date(Number(payload.timestamp) * 1000).toISOString();
  },
 ...
 */
  translateParameters: async function(parameters) {
    ...
    for(idx = 0; idx < parameters.length; idx++) {
      ...
      // for image, video and document, if local file, upload with media api and use the id
      if (['image', 'video', 'document'].includes(param.type)) {
        const value = param[param.type];
        ...
        } else if (validators.filepath(value)) {
          try {
            const uploaded = await fetchers.file(value);
            const media = await chatServer.uploadBuffer({
            ...
            throw `Unable to find media file: ${value}`;
          }
        }
      } else {
        // if nothing applies, leave it unchanged
        translated.push(param);
      }
    }
