...
const GenericBotNode = (platform, RED, factory, configurator) => {
          ...
          when(context.get('currentConversationNode'))
            .then(function(currentConversationNode) {
              ...
              if (currentConversationNode != null) {
                ...
                if (isMaster) {
                  when(context.remove('currentConversationNode', 'currentConversationNode_at'))
                    .then(function () {
...
const GenericOutNode = (platform, RED) => {
  ...
  return function(config) {
    ...
    this.on('input', async function(message, send, done) {
      ...
      // check if valid message or right transport
      if (!isValidMessage(message, node) || getTransport(message) !== platform) {
        done();
        return;
      }
