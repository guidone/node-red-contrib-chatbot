...
var Telegram = new ChatExpress({
  ...
  events: {
    ...
    callback_query: function(botMsg) {
      ...
      connector.answerCallbackQuery(botMsg.id, answer, alert)
        .then(function() {
...
Telegram.in(function(message) {
  return new Promise(function(resolve) {
    if (message.originalMessage.query != null) {
      ...
      message.payload.type = 'inline_query';
      resolve(message);
    } else {
      resolve(message);
    }
...
Telegram.out('request', function(message) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (message.payload.requestType === 'location') {
      keyboard = [
        [{
          text: !_.isEmpty(message.payload.buttonLabel) ? message.payload.buttonLabel : 'Send your position',
      ...
      keyboard = [
        [{
          text: !_.isEmpty(message.payload.buttonLabel) ? message.payload.buttonLabel : 'Send your phone number',
...
Telegram.out('inline-buttons', function(message) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var task = null;
    if (message.originalMessage.modify_message_id != null) {
    ...
    var task = null;
     if (message.originalMessage.modify_message_id != null) {
      task = connector.editMessageText(message.payload.content, {
        ...
        message_id: message.originalMessage.modify_message_id
      });
    } else {
...
Telegram.out('inline_query_answer', function(message) {
  var options = this.getOptions();
  var connector = options.connector;
  return new Promise(function (resolve, reject) {
    ...
    connector.answerInlineQuery(inlineQueryId, message.payload.content)
      .then(
        function() {
          console.log('fatto');
          resolve(message);
