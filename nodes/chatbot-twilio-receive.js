...
var TwilioServer = require('../lib/platforms/twilio/twilio');
...
module.exports = function(RED) {
  ...
  function TwilioBotNode(n) {
    ...
    var twilioConfigs = RED.settings.functionGlobalContext.get('twilio') || {};
    ...
    if (!startNode) {
      ...
      console.log(warn('Twilio Bot ' + this.botname + ' will NOT be launched, environment is ' + environment));
      return;
    }
    // eslint-disable-next-line no-console
    console.log(green('Twilio Bot ' + this.botname + ' will be launched, environment is ' + environment));
    ...
    if (twilioConfigs[node.botname] != null) {
      var validation = validators.platform.twilio(twilioConfigs[node.botname]);
      if (validation != null) {
        /* eslint-disable no-console */
        console.log('');
        console.log(lcd.error('Found a Twilio configuration in settings.js "' + node.botname + '", but it\'s invalid.'));
        ...
        console.log('');
        console.log(lcd.grey('Found a valid Twilio configuration in settings.js: "' + node.botname + '":'));
        console.log(prettyjson.render(twilioConfigs[node.botname]));
        console.log('');
        /* eslint-enable no-console */
        botConfiguration = twilioConfigs[node.botname];
      }
    }
    ...
    if (node.chat == null && botConfiguration.authToken != null) {
      ...
      try {
        ...
        node.chat = TwilioServer.createServer({
  ...
  RED.nodes.registerType('chatbot-twilio-node', TwilioBotNode, {
    credentials: {
      authToken: {
        type: 'text'
      }
  ...
  function TwilioInNode(config) {
    ...
    if (this.config) {
      ...
      if (node.chat) {
        ...
        nodeGlobalKey = 'twilio_master_' + this.config.id.replace('.','_');
        var isMaster = false;
        ...
        node.warn('Missing or incomplete configuration in Twilio Receiver');
      }
    } else {
      node.warn('Missing configuration in Twilio Receiver');
    }
  ...
  RED.nodes.registerType('chatbot-twilio-receive', TwilioInNode);
  ...
  RED.nodes.registerType('chatbot-twilio-send', TwilioOutNode);
